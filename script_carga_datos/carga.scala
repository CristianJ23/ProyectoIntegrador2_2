...
@main
def main():Unit = {
  val pathComplementarios = "C:\\cuarto_semestre\\proyectoIntegrador\\datosGenerados\\dataComplementaria.csv\\100filasComplementraios.csv"
  ...
  val path = "C:\\Users\\carlos\\Downloads\\ddl\\ddl\\proye.csv"
  val reader: CSVReader = CSVReader.open(new File(path))
  val contentFile: List[Map[String, String]] = reader.allWithHeaders()
  val ruta: String = "C:\\intellij\\ProgramadorUtpl\\Proyecto_Integra\\Provincia.csv"
  ...
  val escribi_complementarios=new FileWriter(ruta)
